...
// NEW: Read environment variable for VidKing
...
builder.defineStreamHandler(async (args) => {
    ...
    const providerFetchFunctions = {
        ...
        // VidKing provider with cache integration
        vidking: async () => {
            if (!ENABLE_VIDKING_PROVIDER) {
                console.log('[VidKing] Skipping fetch: Disabled by environment variable.');
                return [];
            }
            if (!shouldFetch('vidking')) {
                console.log('[VidKing] Skipping fetch: Not selected by user.');
                return [];
            }

            // Try to get cached streams first
            const cachedStreams = await getStreamFromCache('vidking', tmdbTypeFromId, tmdbId, seasonNum, episodeNum);
            if (cachedStreams) {
                console.log(`[VidKing] Using ${cachedStreams.length} streams from cache.`);
                return cachedStreams.map(stream => ({ ...stream, provider: 'VidKing' }));
            }

            // No cache or expired, fetch fresh
            try {
                console.log(`[VidKing] Fetching new streams...`);
                const streams = await getVidKingStreams(tmdbId, tmdbTypeFromId, seasonNum, episodeNum);

                if (streams && streams.length > 0) {
                    console.log(`[VidKing] Successfully fetched ${streams.length} streams.`);
                    // Save to cache
                    await saveStreamToCache('vidking', tmdbTypeFromId, tmdbId, streams, 'ok', seasonNum, episodeNum);
                    return streams.map(stream => ({ ...stream, provider: 'VidKing' }));
                } else {
                    console.log(`[VidKing] No streams returned.`);
                    // Save empty result
                    await saveStreamToCache('vidking', tmdbTypeFromId, tmdbId, [], 'failed', seasonNum, episodeNum);
                    return [];
                }
            } catch (err) {
                console.error(`[VidKing] Error fetching streams:`, err.message);
                // Save error status to cache
                await saveStreamToCache('vidking', tmdbTypeFromId, tmdbId, [], 'failed', seasonNum, episodeNum);
                return [];
            }
